 ...
 *
 * a pathfinding plugin based on EasyStarJS (AStar Algorythm)
 * usage : me.plugin.register.defer(this, me.pathfinding.EasyStar, "easystar");
 *
 * you can then use the EasyStarJS API inside your MelonJS game
 *
 *
 */
(function () {
	...
	**/
	 ...
	 * @constructor
	 */
	me.pathfinding.EasyStar = me.plugin.Base.extend(
	/** @scope me.pathfinding.EasyStar.prototype */
	{
	    ...
	    init : function (opts) {
	        ...
	        var options = {
	        	...
	        	cornerCutting: false,
	        	iterations: 1000,
	        	avoidPoints: [], // array of objects of structure {x: 1, y: 2}
	        	additionalPointsCost: [], // array of objects of structure {x: 1, y: 2, cost: 2}
	        	tilesCost: [] // array of objects of structure {tileType: "tileType_name", cost: 2}
	        }
	        ...
	        if(typeof options.iterations === 'number' && options.iterations % 1 === 0 && options.iterations > 0)
	        	this.easystar.setIterationsPerCalculation(options.iterations);

	        if(Object.prototype.toString.call( options.avoidPoints ) === '[object Array]' && options.avoidPoints.length>0)
	        	for (var i = 0; i < options.avoidPoints.length; i++)
	        		this.easystar.avoidAdditionalPoint(options.avoidPoints[i].x, options.avoidPoints[i].y);

	        if(Object.prototype.toString.call( options.additionalPointsCost ) === '[object Array]' && options.additionalPointsCost.length>0)
	        	for (var i = 0; i < options.additionalPointsCost.length; i++)
	        		this.easystar.setAdditionalPointCost(options.additionalPointsCost[i].x, options.additionalPointsCost[i].y, options.additionalPointsCost[i].cost);

	        if(Object.prototype.toString.call( options.tilesCost ) === '[object Array]' && options.tilesCost.length>0)
	        	for (var i = 0; i < options.tilesCost.length; i++)
	        		this.easystar.setTileCost(options.tilesCost[i].tileType, options.tilesCost[i].cost);
			

	        // API
	        this.setGrid = this.easystar.setGrid;
			this.setAcceptableTiles = this.easystar.setAcceptableTiles;
			this.findPath = this.easystar.findPath;
			this.calculate = this.easystar.calculate;
